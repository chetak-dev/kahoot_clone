...
class _HostGameScreenState extends ConsumerState<HostGameScreen> {
  // Length of the pre-game countdown, in seconds. Drives BOTH the synced
  // countdown shown to everyone and the host's local timer that advances to
  // the first question, so they must always use this same value.
  ...
  Future<void> _startGame() async {
    ...
    // Guard against a late/background-throttled fire: only advance if the game
    // is still in the countdown state, so we never reset an already-running
    // quiz back to question 1 (e.g. if participants advanced it while this
    // host tab was backgrounded).
    _countdownTimer?.cancel();
    _countdownTimer = Timer(const Duration(seconds: _countdownSeconds), () {
      if (!mounted) return;
      final session = ref.read(gameSessionProvider(_pin!)).valueOrNull;
          ...
          if (session.status == GameStatus.countdown) {
            return CountdownView(
              endsAt: session.countdownEndsAt ??
                  DateTime.now().add(const Duration(seconds: _countdownSeconds)),
              title: 'Starting in...',
            );
          }
